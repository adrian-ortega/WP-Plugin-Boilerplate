...
/**
 * Returns HTMl Templates in string form to be included in display functions or methods.
 * This requires that the class have a container attached to it
 *
 */
...
trait Templateable
{
    ...
    {
        if(empty($file)) {
            throw new \Exception('Cannot include a file with an empty path or file name.');
        }
        ...
        if(empty($path)) {
            throw new \Exception('Please include a container in your parent class. Preferably use the Runable trait');
        }
        ...
        if(!file_exists($file)) {
            throw new \Exception(sprintf('The template file: `%s` does not exist.', $file));
        }
